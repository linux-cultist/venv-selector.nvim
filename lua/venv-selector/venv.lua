...
function M.activate(hooks, selected_entry)
  ...
  if python_path ~= nil then
    ...
    if count == 0 then
      local message = 'No python lsp servers are running. Please open a python file and then select a venv to activate.'
...
function M.activate_from_cache(settings, venv_info)
  ...
  if venv_type ~= nil and is_conda_family  -- venv_type == 'anaconda' then
